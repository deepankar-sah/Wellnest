...
export default function DashboardLayout() {
  ...
  return (
    <div className={`w-full flex h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} transition-colors duration-300`}>
      {/* Sidebar */}
      <motion.aside 
        className={`w-64 h-screen flex flex-col ${isDarkMode 
          ? 'bg-gray-800/80 text-white border-r border-gray-700' 
          : 'bg-white/80 text-gray-800 border-r border-gray-100'
        } backdrop-blur-md shadow-lg hidden md:flex relative z-10 transition-colors duration-300`}
        initial="hidden"
        animate="visible"
      ...
      >
        <div className="p-6 flex-1 flex flex-col overflow-hidden">
          ...
          <nav className="flex-1 overflow-y-auto pr-2 custom-scrollbar relative z-10">
          ...
          {/* Profile Section */}
          <div className={`pt-4 ${isDarkMode ? 'border-t border-gray-700' : 'border-t border-gray-100'}`}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            ...
            />
            <motion.div
              className={`fixed inset-y-0 left-0 w-64 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} shadow-xl flex flex-col`}
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
            ...
            >
              ...
              {/* Profile Section */}
              <div className={`p-6 ${isDarkMode ? 'border-t border-gray-700' : 'border-t border-gray-100'}`}>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 flex items-center justify-center text-white font-bold">
                    D
                  </div>
      ...
      {/* Main Content */}
      <motion.main 
        className="flex-1 h-screen overflow-y-auto pb-20 md:pb-6 relative"
        initial="hidden"
        animate="visible"
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background: ${isDarkMode ? 'rgba(156, 163, 175, 0.4)' : 'rgba(156, 163, 175, 0.5)'};
          border-radius: 3px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background: ${isDarkMode ? 'rgba(156, 163, 175, 0.6)' : 'rgba(156, 163, 175, 0.7)'};
        }
      `}</style>
    </div>
  );
